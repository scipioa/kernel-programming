...
** Last update Sat Jan 23 13:26:40 2016 Alexandre SCIPION
*/
...
/* void interrupts_set_isr(int n, irq_handler handler, int flags) */
/* { */
/* 	idt_entries[n] */
/* } */

void idt_set_gate(int n, isr_handler handler, int flags)
{
	idt_entries[n].base_lo = handler & 0xFFFF;
        idt_entries[n].base_hi = (handler >> 16) & 0xFFFF;
        idt_entries[n].sel = 0x08;
